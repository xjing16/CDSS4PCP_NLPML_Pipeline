## References:
## https://github.com/DSKSD/DeepNLP-models-Pytorch/blob/master/notebooks/06.RNN-Language-Model.ipynb
## https://colab.research.google.com/github/dpressel/dlss-tutorial/blob/master/1_pretrained_vectors.ipynb#scrollTo=FaQ2BraAR-qI
## https://nlp.gluon.ai/examples/sentiment_analysis/sentiment_analysis.html
## https://madewithml.com/courses/foundations/embeddings/
...
class LanguageModel:
    ...
    def get_integer_seq(self, seq, tokenized=True):
        if tokenized:
            return [self.token2int[w] for w in seq]
        else:
            return [self.token2int[w] for w in seq.split()]

class Trainer:
    def __init__(self, token2int={}, int2token={}, X_train=[], y_train=[], X_test=[], y_test=[]):
        self.token2int = token2int
    ...
    def train(self, net, epochs=10, batch_size=32, lr=0.001, clip=1):
        ...
        for e in range(epochs):
            ...
            for x, y in self.get_batches(self.X_train, self.y_train, batch_size):
        ...
        losses = []
        h = net.init_hidden(batch_size)
        for x, y in self.get_batches(self.X_Test, self.y_test, batch_size):
            inputs, targets = torch.from_numpy(x), torch.from_numpy(y)
            targets = targets.type(torch.LongTensor)
    ...
    def predict(self, net, tkn, h=None):
        ...
        return self.int2token[sampled_token_index], h
    ...
    def saveModel(self, net, file_path):
...
class WordLSTM_v2(nn.Module):
    '''
    Re-train LSTM for export to global neural attention
    '''
    ...
    def __init__(self, n_hidden=256, n_layers=4, drop_prob=0.3, lr=0.001, bidirection=False, pretrainedWE = None, vocab={}):
        ...
        if pretrainedWE==None:
            ...
            ## define the LSTM
            self.lstm = nn.LSTM(200, self.n_hidden//2, n_layers, 
                                dropout=drop_prob, batch_first=True, bidirectional=self.bidirection)
        else:
            self.emb_layer, self.emb_dim = EmbeddingsReader.from_binary(pretrainedWE, vocab)
            self.lstm = nn.LSTM(self.emb_dim, self.n_hidden//2, n_layers, 
                            dropout=drop_prob, batch_first=True, bidirectional=self.bidirection)
    ...
    def init_hidden(self, batch_size):
        ...
        return (torch.randn(self.n_layers*2, batch_size, self.n_hidden // 2).to(DEVICE),
                torch.randn(self.n_layers*2, batch_size, self.n_hidden // 2).to(DEVICE))  
...
if __name__ == "__main__":
    ...
    parser.add_argument("-seqLen","--seqLength", type=int, default = 5, help="To create Train Dataset with sequence length sliding window over sentence. Deafult(5)")
    ...
    net = WordLSTM_v2(n_layers=args.noLayers, bidirection=args.bidirection, pretrainedWE=word_embedding_file, vocab=token2int)
