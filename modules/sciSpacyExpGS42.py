#########################################
## Usage: Run 50 times Experiment Simulation for fine-tuning Base sciSpacy with Random GS 42 + Test on Random GS 91
## Author: Rohan Goli
## Date: 09/20/2022
#########################################
...
def printMetrics(y_targets, y_preds,custom='',experiment=0):
    ...
    with open('sciSpacyExp42GS_results.csv', 'a', newline='') as csvfile:
...
os.system('''
    ...
    python -m spacy init fill-config tempSpacy42GS.cfg config.cfg''')

with open('sciSpacyExp42GS_results.csv', 'w', newline='') as csvfile:
    logwriter = csv.writer(csvfile, delimiter=';',
                            quotechar='|', quoting=csv.QUOTE_MINIMAL)
...
for seed in range(repeat):
    ...
    gsTrainVal, gsTest = train_test_split(gsCombData, test_size=0.684, random_state=seed)
    gsTrain, gsVal = train_test_split(gsTrainVal, test_size=0.20, random_state=seed)
    # print(len(gsTrain),len(gsVal), len(gsTest),sep='\t')
    
    createBERTtSV(gsTrain,'../data/gs42Spacy_T.tsv')
    createBERTtSV(gsVal,'../data/gs42Spacy_V.tsv')

    os.system('''
    source activate pytorch
    cd ../data
    python -m spacy convert gs42Spacy_T.tsv ./ -t json -n 1 -c iob
    python -m spacy convert gs42Spacy_V.tsv ./ -t json -n 1 -c iob
    python -m spacy convert gs42Spacy_T.json ./ -t spacy
    python -m spacy convert gs42Spacy_V.json ./ -t spacy
    cd ../modules
    python -m spacy train -g 0 config.cfg --output ../sciSpacyGS42 --paths.train ../data/gs42Spacy_T.spacy --paths.dev ../data/gs42Spacy_V.spacy''')

    nlp = spacy.load('../sciSpacyGS42/model-best')

    y_preds, y_targets = getTargetPreds(gsTest)
    printMetrics(y_preds, y_targets, ' + 42GS 34 Train + 8 Validation + 91GS Test',seed+1)
